  ...
  return (
    ...
    >
      ...
      <button
        ...
        className={`absolute top-2 right-2 z-20 w-7 h-7 flex items-center justify-center bg-[var(--bg-overlay-90)] backdrop-blur-sm transition-opacity duration-200 md:opacity-0 md:group-hover:opacity-100 ${liked ? "opacity-100" : "opacity-60"}`}
      >
      ...
      <div className="flex flex-col flex-1">
        <Link href={linkHref} className="block px-3 pt-2.5">
          {/* Brand */}
          {showBrand && (
            <p className="text-[9px] tracking-[0.15em] uppercase font-semibold text-[var(--foreground-subtle)] truncate">
              {product.brand}
            </p>
          )}
          {/* Name */}
          ...
          {/* Price */}
          <p className="font-mono text-sm font-medium text-[var(--foreground)] mt-1.5 truncate">
