...
export default function OutfitActions({ outfitId }: OutfitActionsProps) {
  ...
  return (
    <div className="flex flex-col sm:flex-row gap-3 mb-12">
      <button
        onClick={handleLike}
        className={`text-xs tracking-[0.14em] uppercase font-medium px-8 py-4 rounded-xl transition-opacity duration-200 flex items-center justify-center gap-2 ${
          liked
      ...
      <button
        onClick={handleShare}
        className="text-xs tracking-[0.14em] uppercase font-medium text-[var(--foreground)] border border-[var(--border)] px-8 py-4 rounded-xl hover:border-[var(--border-strong)] transition-colors duration-200"
      >
