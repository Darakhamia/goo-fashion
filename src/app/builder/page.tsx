...
export default function BuilderPage() {
  ...
  const [chatMessages, setChatMessages] = useState<ChatMessage[]>([
    { id: "welcome", role: "assistant", text: "Hi! I'm your AI Stylist. Tell me what vibe you're going for, or pick a prompt below." },
  ]);
  ...
  return (
    <div className="h-screen pt-16 flex flex-col overflow-hidden">
      ...
      <div className="flex-1 min-h-0 overflow-hidden relative">
        ...
        ───────────────────────────────────────────────────────────────────── */}
        {aiOpen && (
          <div className="absolute top-0 right-0 bottom-0 w-full md:w-[380px] bg-[var(--background)] border-l border-[var(--border-strong)] flex flex-col z-20 animate-slide-in-right"
                      ...
                      {msg.suggestions.map(product => (
                        ...
                        >
                          <div className="w-[72px] aspect-[3/4] overflow-hidden bg-[var(--surface)] border border-[var(--border)] group-hover:border-[var(--foreground)] transition-colors relative">
                            ...
                            />
                            {(() => {
                              const slot = SLOTS.find(s => s.categories.includes(product.category));
                              const isSelected = slot ? selection[slot.id]?.id === product.id : false;
                              return isSelected ? (
