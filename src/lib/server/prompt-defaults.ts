...
// ── Registry ──────────────────────────────────────────────────────────────────
// Every prompt the studio can override, keyed by its settings row. `required`
// lists the placeholders the generator fills in: a prompt without one of them
// runs without that input (no products, no scraped text, no brief), so an
// override missing any is refused rather than saved.
...
/** The required placeholders a prompt text leaves out, in registry order. */
export function missingPlaceholders(meta: PromptMeta, text: string): string[] {
  return meta.required.filter((p) => !text.includes(p));
}
