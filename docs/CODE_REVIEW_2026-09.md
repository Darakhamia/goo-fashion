...
| # | Блок | Статус |
|---|---|---|
| 1 | Мёртвый код и мусор во всём репозитории | проверен, чинится |
| 2 | Админка: каждая страница — работоспособность, дубли, дизайн, скорость | проверен, чинится |
| 3 | API и безопасность серверной части | впереди (часть закрыта находками блоков 1–2) |
...
13. Мобильная версия админки — открытый вопрос. Пока делаются только дешёвые правки: горизонтальная прокрутка таблиц, одна колонка формы на узком экране.
14. Открыто для CEO: где прод и заведён ли cron `/api/billing/cron/renew` в Coolify; модель цены (`price_min` в двух миграциях 019); graphify (не установлен, хуки его требуют); чистка хранилища от файлов-сирот; обложки блога с чужих сайтов.
